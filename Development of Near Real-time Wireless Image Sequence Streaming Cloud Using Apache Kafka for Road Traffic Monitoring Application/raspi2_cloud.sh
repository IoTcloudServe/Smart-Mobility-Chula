...
sudo ovs-ofctl add-flow br0 arp,priority=100,in_port=LOCAL,arp_tpa=$gateway1_ip,actions="resubmit(,3)"
sudo ovs-ofctl add-flow br0 ip,priority=100,in_port=LOCAL,nw_dst=$gateway1_ip,actions="resubmit(,3)"


sudo ovs-ofctl add-flow br0 arp,priority=100,in_port=LOCAL,arp_tpa=$sdncloud_ip,actions="resubmit(,3)"
sudo ovs-ofctl add-flow br0 ip,priority=100,in_port=LOCAL,nw_dst=$sdncloud_ip,actions="resubmit(,3)"

sudo ovs-ofctl add-flow br0 arp,priority=100,in_port=LOCAL,arp_tpa=$sdncloud_ip,actions="resubmit(,1)"
sudo ovs-ofctl add-flow br0 ip,priority=100,in_port=LOCAL,nw_dst=$sdncloud_ip,actions="resubmit(,1)"

sudo ovs-ofctl add-flow br0 arp,priority=100,in_port=LOCAL,arp_tpa=$gateway1_ip,actions="resubmit(,1)"
sudo ovs-ofctl add-flow br0 ip,priority=100,in_port=LOCAL,nw_dst=$gateway1_ip,actions="resubmit(,1)"


sudo ovs-ofctl add-flow br0 arp,priority=100,in_port=LOCAL,arp_tpa=$raspi1_ip,actions=output:1
sudo ovs-ofctl add-flow br0 arp,priority=100,in_port=LOCAL,arp_tpa=$raspi3_ip,actions=output:1
sudo ovs-ofctl add-flow br0 arp,priority=100,in_port=LOCAL,arp_tpa=$raspi5_ip,actions=output:1
...
sudo ovs-ofctl add-flow br0 table=6,actions=mod_dl_dst:$raspi2,"load:0->OXM_OF_IN_PORT[],resubmit(,5)"

...
#Receive the incoming traffic to Raspi 2 (10.0.0.2) from Raspi 6, Raspi 2 and Raspi 4
...
sudo ovs-ofctl add-flow br0 ip,priority=90,in_port=LOCAL,nw_src=$raspi5_ip,actions="resubmit(,1)"
#Relay the incoming traffic to other wireless nodes not to Raspi 2
sudo ovs-ofctl add-flow br0 arp,priority=90,in_port=1,dl_src=$raspi4,actions="resubmit(,3)"
sudo ovs-ofctl add-flow br0 arp,priority=90,in_port=1,dl_src=$raspi6,actions="resubmit(,4)"
sudo ovs-ofctl add-flow br0 ip,priority=90,in_port=1,dl_src=$raspi4,actions="resubmit(,3)"
sudo ovs-ofctl add-flow br0 ip,priority=90,in_port=1,dl_src=$raspi6,actions="resubmit(,4)"
sudo ovs-ofctl add-flow br0 ip,priority=100,in_port=1,dl_src=$raspi4,nw_dst=$gateway2_ip,actions="resubmit(,3)"
sudo ovs-ofctl add-flow br0 ip,priority=100,in_port=1,dl_src=$raspi6,nw_dst=$raspi4_ip,actions="resubmit(,4)"
sudo ovs-ofctl add-flow br0 ip,priority=100,in_port=1,dl_src=$raspi6,nw_dst=$gateway1_ip,actions="resubmit(,4)"
...
sudo ovs-ofctl add-flow br0 table=3,actions=mod_dl_dst:$raspi6,"load:0->OXM_OF_IN_PORT[],resubmit(,5)"
#Table 4 is to rewrite the destination MAC address into Raspi 3's MAC address
sudo ovs-ofctl add-flow br0 table=4,actions=mod_dl_dst:$raspi4,"load:0->OXM_OF_IN_PORT[],resubmit(,5)"
